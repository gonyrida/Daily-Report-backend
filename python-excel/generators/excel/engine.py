...
def generate_full_report(data, mode="combined"):
    ...
    # Use report-template.xlsx for report mode, template.xlsx for combined mode
    template_name = "report-template.xlsx" if mode == "report" else "template.xlsx"
    ...
    if mode == "report":
        ...
        fill_reference_sheet(
            ws_ref, reference_data, data.get("table_title", "PHOTO REFERENCE")
        )
