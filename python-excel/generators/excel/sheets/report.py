...
from ..images import process_and_insert_logo
...
def fill_report_header(ws, data):
    ...
    # Insert custom logos if provided
    image_cache = {}
    cacpm_logo = data.get("cacpm_logo")
    koica_logo = data.get("koica_logo")

    print(f"DEBUG: CACPM logo provided: {bool(cacpm_logo)}")
    print(f"DEBUG: KOICA logo provided: {bool(koica_logo)}")

    # Clear any existing images in the header area to prevent conflicts
    # This ensures we replace logos without creating duplicates
    ws._images.clear()

    if cacpm_logo:
        print("DEBUG: Inserting CACPM logo")
        try:
            process_and_insert_logo(
                ws,
                cacpm_logo,
                image_cache,
                col_start=1,  # Column B (0-indexed) - left edge
                row_start=0,  # Row 1 (0-indexed)
                width=1.0,    # Single column width
                height=5.0,   # 5 rows (rows 0-4) for B1-B5
                is_left_logo=True,
            )
            print("DEBUG: CACPM logo inserted successfully")
        except Exception as e:
            print(f"DEBUG: Error inserting CACPM logo: {e}")

    if koica_logo:
        print("DEBUG: Inserting KOICA logo")
        try:
            process_and_insert_logo(
                ws,
                koica_logo,
                image_cache,
                col_start=10,  # Column K (0-indexed) - right edge
                row_start=0,   # Row 1 (0-indexed)
                width=1.0,     # Single column width
                height=5.0,    # 5 rows (rows 0-4) for K1-K5
                is_left_logo=False,
            )
            print("DEBUG: KOICA logo inserted successfully")
        except Exception as e:
            print(f"DEBUG: Error inserting KOICA logo: {e}")

    print(f"DEBUG: Final image count in worksheet: {len(ws._images)}")
...
def fill_team_tables(ws, data):
    ...
    try:
        ws.merge_cells(
            start_row=total_row_idx, start_column=2, end_row=total_row_idx, end_column=3
        )
        ws.merge_cells(
            start_row=total_row_idx, start_column=7, end_row=total_row_idx, end_column=8
        )
    except:
        pass
    ...
    ws.cell(row=total_row_idx, column=4).value = (
        f"=SUM(D{start_row}:D{total_row_idx-1})"
    )
    ws.cell(row=total_row_idx, column=5).value = (
        f"=SUM(E{start_row}:E{total_row_idx-1})"
    )
    ws.cell(row=total_row_idx, column=6).value = (
        f"=SUM(F{start_row}:F{total_row_idx-1})"
    )
    ws.cell(row=total_row_idx, column=9).value = (
        f"=SUM(I{start_row}:I{total_row_idx-1})"
    )
    ws.cell(row=total_row_idx, column=10).value = (
        f"=SUM(J{start_row}:J{total_row_idx-1})"
    )
    ws.cell(row=total_row_idx, column=11).value = (
        f"=SUM(K{start_row}:K{total_row_idx-1})"
    )
    ...
    for col in range(2, 12):
        ...
        cell.font = Font(
            name=cell.font.name, size=cell.font.size, bold=True, color=cell.font.color
        )
...
def fill_material_machinery_tables(ws, data, shifted_offset=0):
    ...
    for merge in list(ws.merged_cells.ranges):
        if (
            merge.min_row == header_row or merge.min_row == sub_header_row
        ):  # Added sub_header cleanup
            ws.unmerge_cells(str(merge))

    # 3.2 APPLY HEADER MERGES & TEXT
    ws.merge_cells(
        start_row=header_row, start_column=2, end_row=header_row, end_column=6
    )
    ws.merge_cells(
        start_row=header_row, start_column=7, end_row=header_row, end_column=11
    )

    # 3.3 STYLE THE MAIN HEADER (Materials / Machinery)
    # Using a professional dark blue hex: 4472C4
    header_fill = PatternFill(
        start_color="657C9C", end_color="657C9C", fill_type="solid"
    )
...
def fill_report_sheet(ws, data):
    ...
    ws["B8"] = (
        f"Weather          : AM {data.get('weatherAM', '')}  |  PM {data.get('weatherPM', '')}"
    )
    ws["B9"] = (
        f"Temperature  : AM {data.get('tempAM', '')}°C    |  PM {data.get('tempPM', '')}°C"
    )
